...
let typingChallengeInterval = 5000; // Changed from 15000 to 5000 to start sooner
...
function create() {
    ...
    typingPrompt = this.add.text(400, 200, 'Type "thank" within 10 seconds!', {
    ...
    typingTimerText = this.add.text(400, 250, '10', {
    ...
    typingInput.addEventListener('keyup', function(event) {
        if (event.key === 'Enter' && isTypingChallengeActive) {
            if (typingInput.value.toLowerCase() === 'thank') {
                // Success!
                typingInput.value = '';
                typingInput.style.display = 'none';
                ...
                showMessage('Thank you! +10 points');
                score += 10;
